...
''';
    ...
    try {
      final raw = await _request(prompt: prompt, config: generationConfig);
      ...
      if (decoded is! List) {
        throw const FormatException(
          'Expected the response to be a JSON array.',
        );
      }
      ...
      return decoded.map<TaskDraft>((item) {
        if (item is! Map) {
          throw const FormatException(
            'Each task must be represented as a JSON object.',
          );
        }
      ...
      throw TaskExtractionException(
        'Gemini APIの呼び出しに失敗しました。APIキーと権限を確認してください。',
        e,
      );
