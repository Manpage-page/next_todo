...
void main() {
  group('TaskExtractService.splitTasks', () {
    ...
    test('attempts to extract array even when extra text is returned', () async {
      final service = TaskExtractService.test(
        request: ({required prompt, required config}) async =>
            'Here you go!\n[ {"title": "見積書作成"} ]\nThanks!',
      );
    ...
    test('throws TaskExtractionException when response is not valid JSON', () async {
      final service = TaskExtractService.test(
      ...
      await expectLater(
        ...
        throwsA(
          isA<TaskExtractionException>()
              .having((e) => e.message, 'message', contains('解析できませんでした')),
        ),
      );
    });
    ...
    test('wraps unexpected errors into TaskExtractionException', () async {
      final service = TaskExtractService.test(
        request: ({required prompt, required config}) async => throw Exception('boom'),
      );
      ...
      await expectLater(
        ...
        throwsA(
          isA<TaskExtractionException>()
              .having((e) => e.message, 'message', contains('予期しない')),
        ),
      );
    });
  });
